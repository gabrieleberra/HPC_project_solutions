...
const char* dgemm_desc = "Block matrix multiplication.";
...
// Implementation of blocked matrix multiplication
  ...
  blockSize = 19; // chosen block size
  s = blockSize;
  s_i = blockSize;
  s_j = blockSize;
  s_k = blockSize;
  ...
  for (int ib = 0; ib < N; ib++) {
    ...
    for (int jb = 0; jb < N; jb++) {
      ...
      // Checking the matrix size and handling the irregular blocks on the border
      if (irregFlag) {
        if (jb < N - 1 && ib < N - 1) {
          s_i = blockSize;
          s_j = blockSize;
      ...
      // Checking with kb the position of the box (regular or irregular if it is the last one of the row/column)
