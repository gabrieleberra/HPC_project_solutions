function handle = gplotpartEx6(A,xy,part1,color1,color2,color3)
...
if size(xy,2) == 2
    ...
    offset = 0;
    if max(xy(:,3)) == min(xy(:,3))
        offset = eps;
    end;
    axis([min(xy(:,1)) max(xy(:,1)) min(xy(:,2)) max(xy(:,2)) ...
        min(xy(:,3)) max(xy(:,3))+offset ]); 
end;


hold on
if length(part1) > 0 & length(part2) > 0
    set(gplotgEx6(cut,cutxy,'-'),'color',color3);
    xlabel([int2str(cutsize(A,part1)) ' cut edges'],'visible','on');
else
    xlabel('0 cut edges','visible','on');
end;
if length(part1) > 0 
    set(gplotgEx6(A(part1,part1),xy(part1,:),'-'),'color',color1);
    if n < 500,
        if size(xy,2) == 2
...
if length(part2) > 0 
    set(gplotgEx6(A(part2,part2),xy(part2,:),'-'),'color',color2);
    if n < 500,
        if size(xy,2) == 2
